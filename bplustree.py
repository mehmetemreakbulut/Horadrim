...
class BPlusTree(object):
    ...
    def printTree(self,file):
        ...
        while len(queue) > 0:
            ...
            file.write('Level ' + str(height)+ '|'.join(map(str, node.keys))+ ' -->\t current -> '+ str(node.uid)+
                  '\t parent -> '+
                  str(node.parent.uid) if node.parent else 'Level ' + str(height)+ '|'.join(map(str, node.keys))+ ' -->\t current -> '+ str(node.uid)+
                  '\t parent -> '+
                   'None')
            file.write("\n")
    ...
    def showAllData(self,array):
        ...
        while node:
            ...
            for node_data in node.values:
                data = '[{},{}]'.format(''.join(map(str, str(node.keys[i]))),','.join(map(str, node_data)))+ '->'
                array.append(data)
                i = i+1

            node = node.nextLeaf
        array.append('Last node')
        return array
    ...
    def built(self,fileName):
        ...
        for i in range(len(pages)-1):
            ...
            valuesArray.append(object[1])
        for i in range(len(keysArray)):
            self.insert(keysArray[i], valuesArray[i])
        file.close()
        return self
